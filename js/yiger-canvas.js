...
(function ($) {
  ...
  var StemChart = {
    ...
    drawXY: function (data, key, padding, Ypadding, can, index, config) {
      ...
      for (var i = 0, ptindex; i < 48; i++) {
        ...
        if (i == 0) {
          this.ctx.fillText("0时", x_x-perwidth, x_y, perwidth * 8);
        }
        if (i == 21) {
          this.ctx.fillText("12", x_x+5*tScale, x_y, perwidth * 9);
        }
        if (i == 45) {
          //console.log(x_x-20);
          this.ctx.fillText("24时", x_x+2*tScale, x_y, perwidth * 9);
        }
        if ((i+2)%6 == 0&&i != 46&&i != 22) {
          this.ctx.fillText((i+2)/2, x_x+1*tScale, x_y, perwidth * 9);
        }
      }
      ...
      for (var i = 0; i <= 5; i++) {
        //console.log(i*space);
        //this.ctx.fillStyle = "#38BA9B";//坐标轴坐标颜色
    ...
    drawData: function (config, key, perwidth, yPixel, isMultiData, ymincount, ycount) {
      if (!isMultiData) {
        ...
        for (var i = 0; i < config.data.values[key][keystr].length; i++) {
          ...
          var resultTime = parseInt(result / 60000 / 30)
          ...
          //画最大值
          this.ctx.beginPath();
          this.ctx.arc(x - config.tScale, y - 6 * config.tScale, 3 * config.tScale, 0, Math.PI * 2, false);//绘制数据线上的点
          ...
          if (dotspace > 5) {
            //画最小值
            this.ctx.beginPath();
            this.ctx.arc(x - config.tScale, ymin - 6 * config.tScale, 3 * config.tScale, 0, Math.PI * 2, false);//绘制数据线上的点
            ...
            this.ctx.moveTo(x - config.tScale, ymin - 9 * config.tScale);
    ...
    getCurPixel: function (data, key, width, padding, tScale) {
      var count = 48;
      //return (width-20-padding)/(count+(count-1)*1.5);
  ...
  var CurveChart = {
    ...
    drawXY: function (config, key, can) {
      ...
      var perwidth = this.getStemPixel(config.data, key, can.width, config.padding, config.tScale);//x 轴每一个数据占据的宽度
      var XfontWidth = this.getCurPixel(config.data, key, can.width, config.padding, config.tScale);//x轴坐标宽度
      var ymincount = 0;
      var ycount = 15;
    ...
    drawData: function (config, key, perwidth, yPixel, isMultiData) {
      ...
      var gradient = this.ctx.createLinearGradient(0, 80, 200, 40);
      gradient.addColorStop("0", "#60C8AF");
      gradient.addColorStop("0.8", "#F2D740");
      gradient.addColorStop("1.0", "#F2821E");
      ...
      for (var i = 1; i < config.data.values[key][keystr].length; i++) {
        ...
        var resultTime = parseInt(result / 600000);
        var resultTime0 = parseInt(result0 / 600000);
        ...
        this.ctx.arcTo(x0 - config.tScale, y0 + 5 * config.tScale, x - config.tScale, y + 5 * config.tScale, 1 * config.tScale);
        this.ctx.save();
      }
      this.ctx.stroke();
      this.ctx.closePath();
    ...
    getStemPixel: function (data, key, width, padding, tScale) {
      var count = 144;
      //return (width-20-padding)/(count+(count-1)*1.5);
      return (width - 20 * tScale - padding * 2) / count;
    },//宽度
